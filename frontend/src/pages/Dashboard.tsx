...
// ─── helpers ──────────────────────────────────────────────────────────────────
function StatTile({
  label, value, sub, icon: Icon, gradient, onClick,
}: {
...
export default function Dashboard() {
  ...
  const taskStats = useMemo(() => {
    ...
    return {
      ...
      overdue: allTasks.filter((t) => t.due_date && isPast(new Date(t.due_date)) && !isDone(t.status)).length,
    };
  }, [allTasks]);
        ...
        <div className="rounded-[22px] border border-border/40 bg-card overflow-hidden flex flex-col shadow-sm">
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-border/40">
            {recentTasks.length === 0 ? (
              ...
              return (
                <div key={task.id} className="flex items-center gap-3 px-5 py-3 hover:bg-muted/40 transition-colors duration-200">
                  <Icon className={cn("h-4 w-4 shrink-0", TASK_STATUS_COLOR[task.status])} />
                  <span className="flex-1 text-sm font-medium truncate text-foreground/90">{task.title}</span>
                  {task.due_date && (
                    <span className={cn("text-[10px] shrink-0 font-semibold px-2 py-0.5 rounded-full", overdue ? "bg-red-50 text-red-600 dark:bg-red-950/20" : "bg-slate-50 text-muted-foreground dark:bg-slate-900")}>
          ...
          {/* Mini stats footer */}
          <div className="border-t border-border/40 px-5 py-3.5 grid grid-cols-3 gap-2 bg-secondary/10 shrink-0">
            {[
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="divide-y divide-border/40">
            {topProjects.length === 0 ? (
              ...
              <div
                ...
                className="px-5 py-3.5 hover:bg-muted/40 transition-colors duration-200 cursor-pointer"
              >
                ...
                <div className="flex justify-between mt-2">
                  <span className="text-[10px] font-semibold text-muted-foreground">{p.done}/{p.taskCount} tasks</span>
                  <span className={cn("text-[10px] font-bold capitalize px-2 py-0.5 rounded-full", p.status === "active" ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-950/20" : "bg-slate-50 text-muted-foreground dark:bg-slate-900")}>{p.status}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="rounded-[22px] border border-border/40 bg-card overflow-hidden shadow-sm">
          ...
          <div className="divide-y divide-border/40">
            {activeDeals.length === 0 ? (
              ...
              <div key={deal.id} className="px-5 py-3.5 hover:bg-muted/40 transition-colors duration-200 cursor-pointer" onClick={() => navigate(`/crm/deals/${deal.id}`)}>
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-sm font-semibold truncate flex-1 text-foreground/90">{deal.title}</span>
                  <span className="text-sm font-bold text-foreground ml-2 shrink-0">${Number(deal.value || 0).toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide">{deal.stage || "—"}</span>
                  <div className="flex items-center gap-2">
                    <Progress value={Number(deal.probability) || 0} className="h-1.5 w-16 bg-secondary/85" />
                    <span className="text-[10px] font-bold text-foreground/80">{deal.probability || 0}%</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="rounded-[22px] border border-border/40 bg-card overflow-hidden shadow-sm">
          ...
          <div className="p-5 space-y-4">
            {/* Attendance ring */}
            <div className="flex items-center gap-4 bg-secondary/10 p-3.5 rounded-2xl">
              <div className="relative h-14 w-14 shrink-0">
                <svg className="h-14 w-14 -rotate-90" viewBox="0 0 36 36">
                  <circle cx="18" cy="18" r="15.9" fill="none" stroke="hsl(var(--muted)/40%)" strokeWidth="3" />
                  <circle
              ...
              <div className="space-y-0.5 leading-tight">
                <p className="text-sm font-bold text-foreground">Attendance Today</p>
                ...
                {(hrmsStats?.lateToday ?? 0) > 0 && (
                  <p className="text-[10px] font-bold text-orange-500 mt-1">{hrmsStats.lateToday} late arrivals</p>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                { label: "On Leave", value: hrmsStats?.approvedLeaves ?? "—", color: "text-blue-500" },
                ...
                <div key={s.label} className="rounded-xl bg-secondary/20 px-3 py-2">
                  <p className={cn("text-base font-bold tabular-nums", s.color)}>{s.value}</p>
                  <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">{s.label}</p>
                </div>
              ))}
        ...
        <div className="divide-y divide-border/40">
          {(activities as any[]).length === 0 ? (
            ...
            const BADGE_COLORS: Record<string, string> = {
              ...
              contact: "bg-violet-50 text-violet-600 border-violet-100 dark:bg-violet-950/20 dark:text-violet-400 dark:border-violet-900/30",
              task: "bg-slate-50 text-slate-600 border-slate-100 dark:bg-slate-900 dark:text-slate-400 dark:border-slate-800",
              employee: "bg-orange-50 text-orange-600 border-orange-100 dark:bg-orange-950/20 dark:text-orange-400 dark:border-orange-900/30",
            };
            return (
              <div key={a.id} className="flex items-center gap-3 px-5 py-3 hover:bg-muted/40 transition-colors duration-200">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm truncate text-foreground/90">
                    <span className="font-semibold text-foreground">{a.user_name || "Someone"}</span>
                    <span className="text-muted-foreground font-medium"> {a.activity_type?.replace(/_/g, " ") || "performed an action"}</span>
                    {a.title && <span className="font-semibold text-foreground/95"> — {a.title}</span>}
                  </p>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  <Badge variant="outline" className={cn("text-[9px] uppercase font-bold tracking-wider rounded-md", BADGE_COLORS[badge] || "bg-muted text-muted-foreground border-border")}>
                    {badge}
                  </Badge>
