...
export default function ViewSalarySlipPage() {
  ...
  useEffect(() => {
    ...
    if (absentItem) {
      ...
      if (match) {
        setAbsentsCount(parseInt(match[1], 10));
      } else if (Number(slip.basic_salary) > 0) {
        setAbsentsCount(Math.round(Number(absentItem.amount) / (Number(slip.basic_salary) / 30)));
      }
    } else {
      setAbsentsCount(0);
    }
    ...
    <div className="space-y-6 max-w-4xl mx-auto p-4 sm:p-6">
      <div className="flex items-center justify-between print:hidden">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={handleDownload} className="rounded-xl text-xs font-semibold">
