...
const ExpenseManagement = () => {
  ...
  const renderAvatar = (emp: any) => {
    if (emp.avatar_url) {
      return <img src={emp.avatar_url} className="h-5 w-5 rounded-full object-cover" />;
    }
    return (
  ...
  const filteredEmployees = employees.filter(emp =>
    ...
    (emp.role && emp.role.toLowerCase().includes(empSearch.toLowerCase()))
  );
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList className="grid w-full grid-cols-3 bg-muted/30 p-1 rounded-2xl border border-border/80 w-full">
                  ...
                  <div className="space-y-4 py-3">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        ...
                        <div className="relative">
                          <button
                            type="button"
                            onClick={() => setIsEmpSelectOpen(!isEmpSelectOpen)}
                            className="flex h-9 w-full items-center justify-between rounded-xl border border-input bg-background px-3 py-1.5 text-xs text-foreground"
                          >
                            {expenseForm.employee_id ? (
                              (() => {
                                const emp = employees.find(e => e.id === expenseForm.employee_id);
                                return emp ? (
                                  <div className="flex items-center gap-2">
                                    {renderAvatar(emp)}
                                    <span>{emp.full_name} <span className="text-[10px] text-muted-foreground">({emp.role})</span></span>
                                  </div>
                                ) : "Select claimant";
                              })()
                            ) : "Select claimant"}
                          ...
                          {isEmpSelectOpen && (
                            <div className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-xl border border-border bg-popover p-1 text-popover-foreground shadow-md bg-background">
                              <div className="p-1">
                                <Input
                              ...
                              <div className="space-y-0.5">
                                {filteredEmployees.map(emp => (
                                  <button
                                    ...
                                    type="button"
                                    onClick={() => {
                                      const matchedDept = departments.find(d => d.id.toLowerCase() === emp.department?.toLowerCase());
                                      setExpenseForm({
                                        ...expenseForm,
                                        employee_id: emp.id,
                                        department_id: matchedDept ? matchedDept.id : (emp.department || expenseForm.department_id)
                                      });
                                ...
                                {filteredEmployees.length === 0 && (
                                  <div className="py-2 text-center text-[10px] text-muted-foreground">No employees found</div>
                                )}
                              </div>
                            </div>
                          )}
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Select disabled value={expenseForm.department_id} onValueChange={(val) => setExpenseForm({ ...expenseForm, department_id: val })}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input type="number" placeholder="0.00" value={expenseForm.amount} onChange={(e) => setExpenseForm({ ...expenseForm, amount: e.target.value })} className="bg-background border-input text-foreground rounded-xl text-xs bg-background" />
                      </div>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input placeholder="Describe the purpose of the expense claim" value={expenseForm.description} onChange={(e) => setExpenseForm({ ...expenseForm, description: e.target.value })} className="bg-background border-input text-foreground rounded-xl text-xs bg-background" />
                    </div>
                  </div>
                  <DialogFooter>
                    ...
                    <Button onClick={handleCreateExpense} className="bg-blue-600 hover:bg-blue-600/80 text-white rounded-xl text-xs font-bold">Log Claim</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
            </div>
                ...
                <DialogContent className="bg-background border-border text-foreground rounded-2xl max-w-lg max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="space-y-4 py-3">
                    <div className="space-y-1.5">
                      ...
                      <Input placeholder="e.g. AWS Hosting, Salesforce CRM" value={recurringForm.expense_type} onChange={(e) => setRecurringForm({ ...recurringForm, expense_type: e.target.value })} className="bg-background border-input text-foreground rounded-xl text-xs bg-background" />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <Label className="text-xs font-bold">Amount *</Label>
                        <Input type="number" placeholder="0.00" value={recurringForm.amount} onChange={(e) => setRecurringForm({ ...recurringForm, amount: e.target.value })} className="bg-background border-input text-foreground rounded-xl text-xs bg-background" />
                      </div>
                      <div className="space-y-1.5">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        ...
                        <Input type="date" value={recurringForm.next_due_date} onChange={(e) => setRecurringForm({ ...recurringForm, next_due_date: e.target.value })} className="bg-background border-input text-foreground rounded-xl text-xs bg-background" />
                      </div>
                      <div className="space-y-1.5">
                        <Label className="text-xs font-bold">Currency</Label>
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-xs font-bold">Description / Purpose</Label>
                      <Input placeholder="Describe the recurring subscription" value={recurringForm.description} onChange={(e) => setRecurringForm({ ...recurringForm, description: e.target.value })} className="bg-background border-input text-foreground rounded-xl text-xs bg-background" />
                    </div>
                  </div>
                  <DialogFooter>
                    ...
                    <Button onClick={handleCreateRecurring} className="bg-blue-600 hover:bg-blue-600/80 text-white rounded-xl text-xs font-bold">Setup Recurring</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
            </div>
