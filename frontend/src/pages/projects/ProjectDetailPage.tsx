...
import {
  ...
  Plus, ChevronRight, Flag, CalendarDays,
  LayoutList, Kanban as KanbanIcon, BarChart2, UserCircle2,
  DollarSign, FileText, Paperclip, Bell, Activity as ActivityIcon,
  Circle, ChevronDown, Pencil, Check, X, Trash2, Settings as SettingsIcon,
  Folder, Upload, Download, Trash, UserPlus, Send, Play, Pause,
  Image as ImageIcon, MessageSquare, PlusCircle, Milestone, Shield,
...
import {
  useProject, useUpdateProject, useDeleteProject, useProjectTasks,
  useProjectMembers, useProjectMilestones, useCreateMilestone,
  useUpdateMilestone, useDeleteMilestone, useAddProjectMember, useRemoveProjectMember,
  useProjectComments, useCreateProjectComment, useDeleteProjectComment,
  useProjectActivity, useLogProjectActivity,
...
const STATUS_OPTS = [
  { value: "active", label: "Active", dot: "bg-emerald-500", badge: "bg-emerald-500/10 text-success border-emerald-500/20" },
  { value: "on_hold", label: "On Hold", dot: "bg-yellow-500", badge: "bg-yellow-500/10 text-yellow-400 border-yellow-500/20" },
  { value: "completed", label: "Completed", dot: "bg-blue-500", badge: "bg-blue-500/10 text-primary border-blue-500/20" },
  { value: "cancelled", label: "Cancelled", dot: "bg-red-500", badge: "bg-red-500/10 text-destructive border-red-500/20" },
  { value: "planning", label: "Planning", dot: "bg-violet-500", badge: "bg-violet-500/10 text-violet-400 border-violet-500/20" },
];

const KANBAN_COLS = [
  { id: "backlog", label: "Backlog", color: "text-muted-foreground", dot: "bg-slate-500" },
  { id: "todo", label: "To Do", color: "text-primary", dot: "bg-blue-500" },
  { id: "in_progress", label: "In Progress", color: "text-warning", dot: "bg-amber-500" },
  { id: "review", label: "In Review", color: "text-purple-400", dot: "bg-purple-500" },
  { id: "testing", label: "QA / Testing", color: "text-rose-400", dot: "bg-rose-500" },
  { id: "done", label: "Done", color: "text-emerald-500", dot: "bg-emerald-500" },
];

const PRIORITY_OPTS = [
  { value: "low", label: "Low", color: "text-muted-foreground", dot: "bg-slate-300" },
  ...
  { value: "urgent", label: "Urgent", color: "text-destructive", dot: "bg-red-500" },
];
...
type Section =
  | "overview" | "tasks" | "kanban" | "timeline" | "milestones"
  | "files" | "team" | "discussions" | "activity"
  | "reports" | "settings";
...
export default function ProjectDetailPage() {
  ...
  const { data: projectFiles = [], refetch: refetchFiles } = useProjectFiles(id!);
  ...
  const [editingMilestoneProgress, setEditingMilestoneProgress] = useState(false);
  ...
  useEffect(() => {
    if (project) {
      ...
      setSettingsStart(project.start_date ? project.start_date.slice(0, 10) : "");
  ...
  const handleSelectMention = (member: any) => {
    ...
    if (atIndex !== -1) {
      ...
      const newVal = textBeforeCursor.slice(0, atIndex) + mentionText + textAfterCursor;
  ...
  const handleFileDownload = async (file: any) => {
    try {
      ...
      link.href = blobUrl;
      link.setAttribute('download', file.original_name || file.name || 'download');
  ...
  if (!project || !isUserAuthorized) return (
    <div className="flex flex-col items-center justify-center h-64 text-sm bg-background -m-8 min-h-screen p-8 text-center">
      <div className="max-w-md p-8 rounded-3xl bg-card border border-border shadow-2xl space-y-6">
        ...
        <h2 className="text-xl font-bold text-foreground">Strict Access Mode</h2>
        <p className="text-xs text-muted-foreground leading-relaxed">
          Access Denied. You do not have permission to view this project or its milestones.
          Please contact the project manager or administrator if you believe this is an error.
        </p>
        <Button onClick={() => navigate("/projects")} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-semibold rounded-xl">
          Back to Projects
        </Button>
      </div>
    </div>
  ...
  const done = tasksArray.filter(t => t.status === "done" || t.status === "completed").length;
  const inProgress = tasksArray.filter(t => ["in_progress", "review", "testing"].includes(t.status)).length;
  const todo = tasksArray.filter(t => ["todo", "backlog", "new"].includes(t.status)).length;
  const total = tasksArray.length;

  // Calculate tasks progress
  const tasksProgress = total > 0 ? Math.round(tasksArray.reduce((acc: number, t: any) => acc + (t.progress || 0), 0) / total) : 0;

  // Calculate milestones progress and counts
  const activeMilestones = milestonesArray.filter((m: any) => m.status === "active" || m.status === "in_progress").length;
  const completedMilestones = milestonesArray.filter((m: any) => m.status === "completed" || m.status === "done").length;
  const pendingMilestones = milestonesArray.filter((m: any) => m.status === "pending" || m.status === "planning").length;
  const totalMilestones = milestonesArray.length;
  // Use actual progress values from milestones, not just completion status
  const milestonesProgress = totalMilestones > 0 ? Math.round(milestonesArray.reduce((acc: number, m: any) => acc + (m.progress || 0), 0) / totalMilestones) : 0;

  // Combined project progress (average of tasks and milestones)
  const progressPercent = totalMilestones > 0 && total > 0 ? Math.round((tasksProgress + milestonesProgress) / 2) : (totalMilestones > 0 ? milestonesProgress : tasksProgress);
  const statusOpt = STATUS_OPTS.find(s => s.value === project.status) ?? STATUS_OPTS[0];
  const color = getProjectColor(project);

  const getMemberName = (uid: string | null) =>
    uid ? (orgMembers.find((m: any) => m.id === uid)?.full_name ?? "Unassigned") : "Unassigned";
  ...
  const ganttTasks = tasksArray
    ...
    .map((t, idx) => {
      ...
      return {
        ...
        start,
        duration,
        isOverdue: daysDiff < 0 && t.status !== "completed" && t.status !== "done",
        color: t.status === "done" || t.status === "completed"
  ...
  const moveTaskStatus = (task: any, direction: "next" | "prev") => {
    ...
    if (newIndex !== currentIndex) {
      ...
      updateTask.mutate(
        ...
        { onSuccess: () => { refetchTasks(); logActivity.mutate({ projectId: id!, action: `moved task "${task.title}" to ${nextStatus}`, entity_type: "task", entity_name: task.title }); } }
      );
    }
  };
  ...
  const handleCreateMilestone = () => {
    if (!newMilestoneName.trim()) { toast.error("Milestone name required"); return; }
    if (!newMilestoneDate.trim()) { toast.error("Target date required"); return; }
    createMilestone.mutate({
      project_id: id!,
      name: newMilestoneName.trim(),
      description: newMilestoneDesc,
      due_date: newMilestoneDate,
      assigned_to: newMilestoneAssignee && newMilestoneAssignee !== "none" ? newMilestoneAssignee : null
    }, {
      onSuccess: () => {
        ...
        refetchMilestones();
        logActivity.mutate({ projectId: id!, action: `created milestone "${newMilestoneName}"`, entity_type: "milestone", entity_name: newMilestoneName });
      },
    });
  };
  ...
  const handlePostComment = () => {
    ...
    setCommentText("");
    createComment.mutate({ projectId: id!, comment: text }, {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    uploadFile.mutate({ projectId: id!, file, folder: activeFolder === "All" ? "General" : activeFolder }, {
      onSuccess: () => {
        refetchFiles();
        logActivity.mutate({ projectId: id!, action: `uploaded file "${file.name}"`, entity_type: "file", entity_name: file.name });
  ...
  const handleSaveSettings = () => {
    if (!settingsName.trim()) { toast.error("Project name is required"); return; }
    updateProject.mutate({
      ...
      onSuccess: () => {
        logActivity.mutate({ projectId: id!, action: "updated project settings", entity_type: "project", entity_name: settingsName });
      },
    });
  };
  ...
  const fileFolders = ["All", ...Array.from(new Set(filesArray.map((f: any) => f.folder || "General").filter(Boolean)))];
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] overflow-hidden -m-8 bg-background text-foreground ">
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-background/80 backdrop-blur-xl px-8 py-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3.5 min-w-0">
          <div className={`h-10 w-10 rounded-xl flex items-center justify-center text-white shrink-0 shadow-lg bg-gradient-to-tr ${colorGradients[project.color || "bg-blue-500"] || "from-blue-600 to-blue-500"}`}>
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <Badge variant="outline" className={cn("text-[10px] font-bold uppercase tracking-wider border-none shrink-0", statusOpt.badge)}>
                {project.status || "active"}
              </Badge>
            </div>
            <p className="text-xs text-muted-foreground mt-0.5">
              {total} tasks <span className="text-primary font-bold text-[20px]">.</span> {totalMilestones} miles <span className="text-primary font-bold text-[20px]">.</span> {progressPercent}% done
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-5 shrink-0">
          <div className="hidden md:flex flex-col items-end gap-1">
            ...
            <Progress value={progressPercent} className="h-1.5 w-32 bg-accent rounded-full" />
          </div>

          <div className="flex items-center gap-2">
            <div className="flex -space-x-1.5">
              {involvedMembersList.slice(0, 3).map((m: any, index: number) => (
                <Avatar key={m.id || index} className="h-7 w-7 border-2 border-[#0B1220]">
            ...
            <Button size="icon" onClick={() => setSection("settings")}
      ...
      <div className="flex-1 overflow-y-auto p-8 bg-background/20">
          ...
          <div className="space-y-6 max-w-7xl animate-in fade-in duration-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              ...
              <Card className="border-border bg-card/40 backdrop-blur-xl rounded-2xl p-5 flex items-center justify-between shadow-lg">
                <div className="space-y-1.5">
                  <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold">Project Stance</span>
                  <div className="text-2xl font-bold text-foreground">{progressPercent}%</div>
                  <div className="text-[10px] text-success font-bold uppercase tracking-wider">Operational Health</div>
                </div>
                <div className="relative h-14 w-14 flex items-center justify-center shrink-0">
                  <svg className="absolute transform -rotate-90 w-full h-full">
                    <circle cx="28" cy="28" r="22" stroke="currentColor" className="text-border" strokeWidth="4" fill="transparent" />
                    <circle cx="28" cy="28" r="22" stroke="currentColor" className="text-primary" strokeWidth="4" fill="transparent"
              ...
              <Card className="border-border bg-card/40 backdrop-blur-xl rounded-2xl p-5 shadow-lg space-y-2">
                <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold">Tasks Stats</span>
                <div className="grid grid-cols-3 gap-2 pt-1">
                  <div className="p-2 rounded-xl bg-muted/40 border border-border/80 text-center">
                    <div className="text-xs font-bold text-foreground/80">{todo}</div>
                    <div className="text-[10px] text-muted-foreground uppercase font-bold mt-0.5">Todo</div>
                  </div>
                  <div className="p-2 rounded-xl bg-amber-500/5 border border-amber-500/10 text-center">
                    <div className="text-xs font-bold text-warning">{inProgress}</div>
                    <div className="text-[10px] text-muted-foreground uppercase font-bold mt-0.5">Active</div>
                  </div>
                  <div className="p-2 rounded-xl bg-emerald-500/5 border border-emerald-500/10 text-center">
                    <div className="text-xs font-bold text-emerald-500">{done}</div>
                    <div className="text-[10px] text-muted-foreground uppercase font-bold mt-0.5">Done</div>
                  </div>
                </div>
                <div className="pt-1">
                  ...
                  <Progress value={tasksProgress} className="h-1.5 bg-muted rounded-full" />
                </div>
              </Card>

              {/* Milestones Stats */}
              {(() => {
                const milestoneProgress = totalMilestones > 0 ? Math.round(milestonesArray.reduce((acc: number, m: any) => acc + (m.progress || 0), 0) / totalMilestones) : 0;

                return (
                  <Card className="border-border bg-card/40 backdrop-blur-xl rounded-2xl p-5 shadow-lg space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold">Milestones</span>
                      {isAdmin && (
                        <Button size="icon" variant="ghost" onClick={() => setShowMilestoneDialog(true)}
                    ...
                    <div className="grid grid-cols-3 gap-2 pt-1">
                      <div className="p-2 rounded-xl bg-muted/40 border border-border/80 text-center">
                        <div className="text-xs font-bold text-foreground/80">{pendingMilestones}</div>
                        <div className="text-[10px] text-muted-foreground uppercase font-bold mt-0.5">Todo</div>
                      </div>
                      <div className="p-2 rounded-xl bg-amber-500/5 border border-amber-500/10 text-center">
                        <div className="text-xs font-bold text-warning">{activeMilestones}</div>
                        <div className="text-[10px] text-muted-foreground uppercase font-bold mt-0.5">Active</div>
                      </div>
                      <div className="p-2 rounded-xl bg-emerald-500/5 border border-emerald-500/10 text-center">
                        <div className="text-xs font-bold text-emerald-500">{completedMilestones}</div>
                        <div className="text-[10px] text-muted-foreground uppercase font-bold mt-0.5">Done</div>
                      </div>
                    </div>
                    <div className="pt-1">
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-[10px] font-bold text-muted-foreground uppercase">Progress</span>
                        <span className="text-[10px] font-bold text-foreground">{milestoneProgress}%</span>
                      </div>
                      <Progress value={milestoneProgress} className="h-1.5 bg-muted rounded-full" />
                    </div>
                  </Card>
                );
              })()}
              ...
              {project.budget && (
                <Card className="border-border bg-card/40 backdrop-blur-xl rounded-2xl p-5 shadow-lg space-y-3">
                  <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold">Budget</span>
                  <div className="flex items-baseline justify-between">
                    <span className="text-2xl font-bold text-success">
                      ${(project.budget - (project.spent_budget || 0)).toLocaleString()}
                    </span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols gap-6">
              <Card className="lg:col-span-2 border-border bg-card/30 backdrop-blur-md rounded-2xl p-6 shadow-md space-y-5">
                <div>
                  <h3 className="text-xs uppercase tracking-wider font-bold text-foreground/80">Project Brief & Parameters</h3>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2">
                  <div className="space-y-1">
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">Initiated</span>
                    <span className="text-xs font-bold text-foreground">
                      {project.start_date ? format(new Date(project.start_date), "MMM d, yyyy") : "Not set"}
                    </span>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">Target Due</span>
                    <span className="text-xs font-bold text-foreground">
                      {project.end_date ? format(new Date(project.end_date), "MMM d, yyyy") : "Flexible"}
                    </span>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">Project Manager</span>
                    <span className="text-xs font-bold text-foreground">
                      {getMemberName(project.manager_id) !== "Unassigned" ? getMemberName(project.manager_id) : (project.created_by_name || "Owner")}
                    </span>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">Team Size</span>
                    <span className="text-xs font-bold text-primary">{involvedMembersList.length} {involvedMembersList.length === 1 ? 'member' : 'members'}</span>
                  </div>
                </div>
              </Card>
        ...
        {section === "tasks" && (
          <div className="space-y-4 max-w-7xl animate-in fade-in duration-200">
            <div className="flex justify-between items-center bg-card/40 backdrop-blur-md px-6 py-4 rounded-xl border border-border">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Tasks Ledger</span>
                <span className="ml-3 text-[10px] text-muted-foreground/70">{total} tasks · {done} completed</span>
              </div>
              <Button size="sm" onClick={() => setShowTaskDialog(true)} className="gap-1.5 h-8 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold">
                ...
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-border">
                    {tasksArray.map((t: any) => {
                      ...
                      const priorityColor = { urgent: "text-destructive", high: "text-orange-400", medium: "text-yellow-400", low: "text-muted-foreground" }[t.priority as string] || "text-muted-foreground";
                      ...
                      return (
                        <tr key={t.id} onClick={() => setSelectedTaskForDrawer(t)}
                          className="hover:bg-accent/20 transition-all cursor-pointer group">
                          <td className="p-4 text-center" onClick={e => { e.stopPropagation(); handleToggleTaskStatus(t); }}>
                            <button className={cn("hover:scale-110 transition-transform bg-transparent", isDone ? "text-success" : "text-muted-foreground/70")}>
                              {isDone ? <CheckCircle2 className="h-4 w-4" /> : <Circle className="h-4 w-4" />}
                            </button>
                          </td>
                          <td className="p-4 font-bold text-foreground">
                            <span className={cn(isDone ? "line-through text-muted-foreground/70" : "")}>{t.title}</span>
                          ...
                          <td className="p-4">
                            <div className="flex items-center gap-2">
                              <Avatar className="h-5 w-5 border border-border">
                                {t.assigned_to_avatar && <AvatarImage src={t.assigned_to_avatar} alt={t.assigned_to_name || ""} />}
                                <AvatarFallback className="text-[10px] font-bold bg-blue-500/10 text-primary">
                                  {getInitials(t.assigned_to_name || getMemberName(t.assigned_to))}
                                </AvatarFallback>
                              </Avatar>
                              <span className="text-muted-foreground text-[10px]">{t.assigned_to_name || getMemberName(t.assigned_to)}</span>
                            </div>
                          </td>
                          <td className="p-4">
                            <span className={cn("text-[10px] font-bold uppercase", priorityColor)}>{t.priority || "normal"}</span>
                          </td>
                    ...
                    {tasksArray.length === 0 && (
                      <tr>
                        <td colSpan={7} className="text-center py-16 text-muted-foreground">
                          ...
                          <p className="font-bold text-foreground/80">No tasks yet</p>
                          <p className="text-xs mt-1">Click "Add Task" to create your first project task.</p>
                        </td>
                      </tr>
                    )}
                  </tbody>
        ...
        {section === "kanban" && (
          <div className="space-y-4 max-w-full h-full flex flex-col animate-in fade-in duration-200">
            <div className="flex justify-between items-center bg-card/40 backdrop-blur-md px-6 py-4 rounded-xl border border-border shrink-0">
              <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Kanban Board</span>
              <Button size="sm" onClick={() => { setNewTaskStatus("todo"); setShowTaskDialog(true); }} className="gap-1.5 h-8 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold">
                <Plus className="h-4 w-4" /> Add Task
              </Button>
            </div>
          ...
          <div className="space-y-4 max-w-7xl animate-in fade-in duration-200">
            <Card className="border-border bg-card/30 backdrop-blur-md rounded-2xl p-6 shadow-lg">
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-[10px] text-muted-foreground">{ganttTasks.length} tasks with due dates</span>
              </div>

              {ganttTasks.length === 0 ? (
                <div className="text-center py-16 text-xs text-muted-foreground">
                  <CalendarDays className="h-10 w-10 mx-auto mb-3 opacity-20" />
                  <p className="font-bold text-foreground/80">No tasks with due dates</p>
                  <p className="mt-1">Set due dates on tasks to visualize the timeline.</p>
                </div>
              ) : (
                <div className="space-y-3 overflow-x-auto">
                  ...
                  <div className="space-y-2.5 pb-2">
                    {ganttTasks.map(gt => (
                      <div key={gt.id} className="flex items-center gap-4 text-xs min-w-[700px]">
                        <div className="w-52 shrink-0">
                          <div className="font-bold text-foreground/80 truncate">{gt.title}</div>
                          {gt.dueDate && (
                            <div className={cn("text-[10px] mt-0.5", gt.isOverdue ? "text-destructive" : "text-muted-foreground/70")}>
                              Due: {format(new Date(gt.dueDate), "MMM d")} {gt.isOverdue && "· OVERDUE"}
                            </div>
                          )}
                        </div>
                        <div className="flex-1 h-9 bg-muted/20 rounded-xl border border-border/40 relative">
                          <div
                            className={cn("absolute top-1 h-7 rounded-lg border flex items-center px-2.5 font-bold text-[10px] shadow-sm truncate transition-all", gt.color)}
                            style={{
            ...
            {milestonesArray.length > 0 && (
              <Card className="border-border bg-card/30 backdrop-blur-md rounded-2xl p-6 shadow-md">
                <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-4 flex items-center gap-2">
                  <Milestone className="h-4 w-4 text-violet-500" /> Milestone Checkpoints
                </h3>
                <div className="space-y-3">
                  {milestonesArray.map((m: any) => (
                    <div key={m.id} className="flex items-center gap-4">
                      <div className={cn("h-3 w-3 rounded-full shrink-0", m.status === "completed" ? "bg-emerald-500" : "bg-violet-500")} />
                      <div className="flex-1 flex items-center justify-between">
                        <span className={cn("text-xs font-bold", m.status === "completed" ? "text-muted-foreground/70 line-through" : "text-foreground")}>
                          {m.name}
                        </span>
                        <div className="flex items-center gap-3">
                          <span className="text-[10px] text-muted-foreground">{m.due_date ? format(new Date(m.due_date), "MMM d, yyyy") : "No date"}</span>
                          <Badge variant="outline" className={cn("text-[10px] border-none font-bold uppercase",
                            m.status === "completed" ? "bg-emerald-500/10 text-success" : "bg-violet-500/10 text-violet-400")}>
                            {m.status}
                          </Badge>
                        </div>
                      </div>
        ...
        {section === "milestones" && (() => {
          const milestonesArray = Array.isArray(milestones) ? milestones : [];
          const activeMilestone = milestonesArray.find((m: any) => m.id === selectedMilestone?.id) || selectedMilestone;
          const isMilestoneOwner = project && (project.created_by === profile?.id || project.manager_id === profile?.id || project.owner_id === profile?.id);
          const isMilestoneAssignee = activeMilestone && activeMilestone.assigned_to === profile?.id;
          const isRestrictedMilestoneUser = !isAdmin && !isMilestoneOwner && isMilestoneAssignee;
          const milestoneComments = activeMilestone
            ? comments.filter((c: any) => c.entity_type === 'milestone' && c.entity_id === activeMilestone.id)
            : [];
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-7xl animate-in fade-in duration-200">
              ...
              <div className="lg:col-span-1 space-y-4">
                <div className="flex justify-between items-center bg-card/40 backdrop-blur-md px-5 py-3.5 rounded-xl border border-border">
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground block">Project Road</span>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-xs font-bold text-foreground">{milestonesArray.length} Milestones</span>
                      {milestonesArray.length > 0 && (
                        <div className="flex items-center gap-1.5 text-[10px]">
                          {pendingMilestones > 0 && <span className="text-muted-foreground">{pendingMilestones} Planning</span>}
                          {activeMilestones > 0 && (
                            <>
                              {pendingMilestones > 0 && <span className="text-muted-foreground/50">·</span>}
                              <span className="text-amber-400 font-bold">{activeMilestones} Active</span>
                            </>
                          )}
                          {completedMilestones > 0 && (
                            <>
                              {(pendingMilestones > 0 || activeMilestones > 0) && <span className="text-muted-foreground/50">·</span>}
                              <span className="text-emerald-400 font-bold">{completedMilestones} Done</span>
                            </>
                          )}
                        </div>
                      )}
                  ...
                  <Button size="sm" onClick={() => setShowMilestoneDialog(true)} className="gap-1.5 h-8 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold">
                ...
                <div className="space-y-2.5 max-h-[550px] overflow-y-auto pr-1">
                  {milestonesArray.map((m: any) => {
                    ...
                    const assignee = involvedMembersList.find((mem: any) => mem.id === m.assigned_to);

                    return (
                      <Card
                        key={m.id}
                        onClick={() => { setSelectedMilestone(m); setMilestoneCommentText(""); }}
                        className={cn(
                          ...
                          isSelected && "border-blue-500/60 bg-blue-500/5 hover:bg-blue-500/5 ring-1 ring-blue-500/30"
                        )}
                      >
                        <div className="flex items-start justify-between gap-3 mb-2.5">
                          ...
                          <Badge variant="outline" className={cn("text-[9px] border-none font-bold uppercase shrink-0 px-2 py-0.5",
                            m.status === "completed" ? "bg-emerald-500/10 text-success" :
                              m.status === "in_progress" ? "bg-amber-500/10 text-warning" : "bg-slate-500/10 text-muted-foreground")}>
                        ...
                        <div className="flex items-center justify-between mt-auto">
                          <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground/80">
                            ...
                            <span>{m.due_date ? format(new Date(m.due_date), "MMM d, yyyy") : "No due date"}</span>
                          </div>
                          ...
                          {assignee ? (
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-[10px] font-bold text-muted-foreground max-w-[80px] truncate">{assignee.full_name.split(' ')[0]}</span>
                            </div>
                          ) : (
                        ...
                        <div className="w-full bg-slate-800/60 h-1 rounded-full mt-3 overflow-hidden">
                          <div
                            className={cn("h-full rounded-full transition-all duration-300", m.status === 'completed' ? "bg-emerald-500" : "bg-blue-500")}
                  ...
                  {milestonesArray.length === 0 && (
                    <div className="text-center py-16 text-muted-foreground/80 text-xs">
                      ...
                      <p className="font-bold text-foreground/80">No milestones yet</p>
                      <p className="mt-1">Define project checkpoints to track progress.</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="lg:col-span-2">
                {!activeMilestone ? (
                  <Card className="border-border bg-card/20 backdrop-blur-md rounded-2xl p-16 text-center shadow-md h-full flex flex-col justify-center items-center min-h-[300px]">
                    ...
                    <h4 className="text-xs font-bold text-foreground">Select a Milestone</h4>
                    <p className="text-[10px] text-muted-foreground max-w-xs mt-1">
                      Choose a milestone from the list to view its workload parameters, assignee details, and team comments.
                    </p>
                  </Card>
                ) : (
                  <div className="space-y-4">
                    ...
                    <Card className="border-border bg-card/30 backdrop-blur-md rounded-2xl p-5 shadow-md space-y-4">
                      <div className="flex justify-between items-start gap-4">
                        <div>
                          <h3 className="text-sm font-bold text-foreground flex items-center gap-2">
                            <Milestone className="h-4 w-4 text-primary" /> {activeMilestone.name}
                          </h3>
                        ...
                        <Button
                          ...
                          size="icon"
                          onClick={() => {
                            deleteMilestone.mutate({ id: activeMilestone.id, project_id: id! }, {
                              onSuccess: () => {
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 border-t border-border/50">
                        {/* Status Select */}
                        <div className="space-y-1.5">
                          <Label className="text-[10px] font-bold text-muted-foreground uppercase">Milestone Status</Label>
                          <Select
                            value={activeMilestone.status || "pending"}
                            onValueChange={(newStatus) => {
                              const updateData: any = { id: activeMilestone.id, project_id: id!, status: newStatus };
                          ...
                          >
                            <SelectTrigger className="text-xs bg-muted border-border text-foreground h-9"><SelectValue /></SelectTrigger>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <Select
                            value={activeMilestone.assigned_to || "none"}
                            disabled={isRestrictedMilestoneUser}
                            onValueChange={(newAssignee) => {
                              updateMilestone.mutate({ id: activeMilestone.id, project_id: id!, assigned_to: newAssignee === "none" ? null : newAssignee }, {
                                onSuccess: () => {
                                  refetchMilestones();
                                }
                              });
                          ...
                          >
                            ...
                            <SelectContent className="bg-muted border-border text-xs">
                              ...
                              {orgMembers && Array.isArray(orgMembers) && orgMembers.map((m: any) => (
                        ...
                        {/* Target Date */}
                        <div className="space-y-1.5">
                          <Label className="text-[10px] font-bold text-muted-foreground uppercase">Target Due Date</Label>
                          <Input
                            type="date"
                            disabled={isRestrictedMilestoneUser}
                            value={activeMilestone.due_date ? activeMilestone.due_date.slice(0, 10) : ""}
                            onChange={(e) => {
                              updateMilestone.mutate({ id: activeMilestone.id, project_id: id!, due_date: e.target.value || null }, {
                      ...
                      <div className="space-y-2 pt-2 border-t border-border/50">
                        <div className="flex justify-between items-center text-[10px] font-bold text-muted-foreground uppercase">
                          ...
                          <span className="text-primary font-bold">{editingMilestoneProgress ? tempMilestoneProgress : activeMilestone.progress || 0}%</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Slider
                            value={[editingMilestoneProgress ? tempMilestoneProgress : activeMilestone.progress || 0]}
                            onValueChange={(val) => {
                          ...
                          {!editingMilestoneProgress ? (
                            <Button
                              ...
                              onClick={() => {
                                ...
                                setTempMilestoneProgress(activeMilestone.progress || 0);
                              }}
                            ...
                            <>
                              <Button
                                size="sm"
                                onClick={() => {
                                  updateMilestone.mutate({ id: activeMilestone.id, project_id: id!, progress: tempMilestoneProgress }, {
                                    onSuccess: () => {
                                      refetchMilestones();
                                ...
                                className="h-8 px-3 bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-bold"
                              >
                                Save
                              </Button>
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => setEditingMilestoneProgress(false)}
                                className="h-8 w-8 p-0"
                              >
                                <X className="h-3 w-3" />
                              </Button>
                    ...
                    <Card className="border-border bg-card/30 backdrop-blur-xl rounded-2xl flex flex-col h-[400px] shadow-lg overflow-hidden">
                      <div className="px-5 py-3 border-b border-border/80 bg-muted/10 flex items-center justify-between shrink-0">
                        <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                          <MessageSquare className="h-4 w-4 text-primary" /> Daily Work Logs & updates
                        </h3>
                        <span className="text-[9px] text-muted-foreground">{milestoneComments.length} logs posted</span>
                      </div>
                      ...
                      <div className="flex-1 overflow-y-auto p-5 space-y-3.5">
                        {milestoneComments.length === 0 ? (
                          <div className="text-center py-12 text-muted-foreground">
                            ...
                            <h4 className="text-xs font-bold text-foreground">No updates logged yet</h4>
                            <p className="text-[9px] text-muted-foreground/80 mt-1 max-w-xs mx-auto">
                              The milestone assignee and project team should post daily progress updates here.
                            </p>
                          </div>
                        ) : (
                              ...
                              <div className="bg-card/50 border border-border/80 p-3 rounded-2xl rounded-tl-none flex-1 relative group">
                                <div className="flex justify-between items-baseline mb-1">
                                  <div className="flex items-baseline gap-2">
                                    <span className="text-[11px] font-bold text-foreground">{comm.full_name || "Team Member"}</span>
                                    <span className="text-[9px] text-muted-foreground/60">
                                      {comm.created_at ? formatDistanceToNow(new Date(comm.created_at), { addSuffix: true }) : "Just now"}
                                    </span>
                                  </div>
                                  {comm.user_id === profile?.id && (
                                    <button
                                      onClick={() => deleteComment.mutate({ id: comm.id, projectId: id! }, { onSuccess: () => refetchComments() })}
                                ...
                                <p className="text-xs text-foreground/90 leading-relaxed font-normal">{comm.comment}</p>
                              </div>
                            </div>
                          ))
                        )}
                      ...
                      <div className="p-3 border-t border-border/80 bg-background/40 flex gap-2 items-end shrink-0">
                        <Textarea
                          ...
                          onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handlePostMilestoneComment(); } }}
                        ...
                        <Button
                          ...
                          disabled={!milestoneCommentText.trim() || createComment.isPending}
                          size="sm"
                          className="h-9 rounded-xl font-bold bg-blue-600 hover:bg-blue-500 text-white shrink-0"
                        >
        ...
        {section === "files" && (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-5 max-w-7xl animate-in fade-in duration-200">
            ...
            <div className="space-y-1 lg:col-span-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground block px-2 mb-3">Folders</span>
              {fileFolders.map(folder => (
                <button key={folder} onClick={() => setActiveFolder(folder)}
                  ...
                  <Badge variant="secondary" className="bg-accent text-muted-foreground border-none scale-90">
                    {folder === "All" ? filesArray.length : filesArray.filter((f: any) => f.folder === folder).length}
                  </Badge>
                </button>
              ))}
            </div>
            ...
            <div className="lg:col-span-3 space-y-4">
              <div className="flex justify-between items-center bg-card/40 backdrop-blur-md px-5 py-4 rounded-xl border border-border">
                <div>
                  <span className="text-xs font-bold text-foreground">{activeFolder}</span>
                  <span className="text-[10px] text-muted-foreground/70 ml-2">
                    {activeFolder === "All" ? filesArray.length : filesArray.filter((f: any) => f.folder === activeFolder).length} files
                  </span>
                </div>
                <div>
                  <input type="file" id="project-vault-upload" ref={fileInputRef} className="hidden" onChange={handleFileUpload} />
                  <Button size="sm" asChild className="gap-1.5 h-8 rounded-xl font-bold bg-blue-600 hover:bg-blue-500 text-white">
                    <label htmlFor="project-vault-upload" className="cursor-pointer">
              ...
              <Card className="border-border bg-card/30 backdrop-blur-md rounded-2xl overflow-hidden shadow-md">
                <CardContent className="p-0">
                  {filesArray.filter((f: any) => activeFolder === "All" || f.folder === activeFolder).length === 0 ? (
                    <div className="text-center py-16 text-muted-foreground">
                      <Upload className="h-10 w-10 mx-auto mb-3 opacity-20" />
                      <p className="font-bold text-foreground/80">No files yet</p>
                      <p className="text-xs mt-1">Upload project documents, designs, and contracts here.</p>
                    </div>
                  ) : (
                    <div className="divide-y divide-slate-800/40">
                      {filesArray
                        .filter((f: any) => activeFolder === "All" || f.folder === activeFolder)
                        .map((file: any) => (
                          <div key={file.id} className="flex items-center justify-between p-4 hover:bg-accent/5 transition-colors">
                            <div className="flex items-center gap-3.5 min-w-0">
                              ...
                              <div className="min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-1 text-[10px] text-muted-foreground">
                                  {file.version && <span className="bg-accent px-1.5 py-0.5 rounded font-bold text-[10px] uppercase text-foreground/80">{file.version}</span>}
                                  <span>{file.size || (file.size_bytes ? formatFileSize(file.size_bytes) : "")}</span>
                                  <span>·</span>
                                  <span>By: {file.uploadedBy || file.uploaded_by_name || "Team"}</span>
                                  <span>·</span>
                                  <span>{file.date || (file.created_at ? format(new Date(file.created_at), "MMM d, yyyy") : "")}</span>
                                </div>
                              </div>
                            </div>
                            <div className="flex items-center gap-2">
                              <Button variant="ghost" size="icon" onClick={() => handleFileDownload(file)} className="hover:bg-accent h-8 w-8 text-muted-foreground hover:text-foreground bg-transparent">
                                <Download className="h-4 w-4" />
                              </Button>
                              {(profile?.id === file.uploaded_by || userRole?.role === "admin" || userRole?.role === "super_admin") && (
                                <Button variant="ghost" size="icon"
                                  onClick={() => deleteFile.mutate({ id: file.id, projectId: id! }, { onSuccess: () => refetchFiles() })}
          ...
          <div className="space-y-4 max-w-7xl animate-in fade-in duration-200">
            <div className="flex justify-between items-center bg-card/40 backdrop-blur-md px-5 py-4 rounded-xl border border-border">
              <div>
                ...
                <span className="text-[10px] text-muted-foreground/70 ml-3">{membersArray.length} members</span>
              </div>
              <Button size="sm" onClick={() => setShowMemberDialog(true)} className="gap-1.5 h-8 rounded-xl font-bold bg-blue-600 hover:bg-blue-500 text-white">
            ...
            {membersArray.length === 0 ? (
              <Card className="border-border bg-card/30 backdrop-blur-md rounded-2xl p-12 text-center shadow-md">
                ...
                <p className="font-bold text-foreground/80">No team members yet</p>
                <p className="text-xs text-muted-foreground/70 mt-1">Invite colleagues to collaborate on this project.</p>
              </Card>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {membersArray.map((m: any) => {
                  const activeCount = tasksArray.filter(t => t.assigned_to === m.user_id).length;
                  const doneCount = tasksArray.filter(t => t.assigned_to === m.user_id && (t.status === "done" || t.status === "completed")).length;
                  const workload = activeCount > 0 ? Math.min(100, Math.round((activeCount / Math.max(total, 1)) * 100 * membersArray.length)) : 0;
                  return (
                    <Card key={m.id} className="border-border bg-card/30 backdrop-blur-md rounded-2xl p-5 shadow-md">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h4 className="text-xs font-bold text-foreground">{m.full_name || "Employee"}</h4>
                            <span className="text-[10px] text-muted-foreground capitalize mt-0.5 block">{m.email || ""}</span>
                            <Badge variant="outline" className="mt-1 text-[10px] font-bold uppercase border-slate-700 text-muted-foreground px-1.5 py-0.5">
                              {m.role || "Member"}
                            </Badge>
                          </div>
                        </div>
                        <Button variant="ghost" size="icon"
                          onClick={() => removeMember.mutate({ id: m.id, project_id: id! }, { onSuccess: () => refetchMembers() })}
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-[10px] uppercase font-bold text-muted-foreground">
                          ...
                          <span className="text-foreground">{activeCount} assigned · {doneCount} done</span>
                        </div>
                        <Progress value={activeCount > 0 ? (doneCount / activeCount) * 100 : 0} className="h-1.5 bg-accent rounded-full" />
                      </div>
                    </Card>
                  );
                })}
          ...
          <div className="max-w-4xl animate-in fade-in duration-200">
            <Card className="border-border bg-card/30 backdrop-blur-xl rounded-2xl flex flex-col h-[600px] shadow-lg overflow-hidden">
              <div className="px-6 py-4 border-b border-border/80 bg-muted/10 flex items-center justify-between shrink-0">
                <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                  <MessageSquare className="h-4 w-4 text-primary" /> Project Discussions
                </h3>
                <Button variant="ghost" size="icon" onClick={() => refetchComments()} className="h-7 w-7 text-muted-foreground/70 hover:text-foreground/80 bg-transparent">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {loadingComments ? (
                  <div className="text-center py-8 text-xs text-muted-foreground/70">Loading discussions...</div>
                ) : commentsArray.length === 0 ? (
                  <div className="text-center py-16 text-muted-foreground">
                    <MessageSquare className="h-10 w-10 mx-auto mb-3 opacity-20" />
                    <h4 className="text-xs font-bold text-foreground">Start the conversation</h4>
                    <p className="text-[10px] text-muted-foreground mt-1">Post a message to discuss project updates with your team.</p>
                  </div>
                ) : (
                    ...
                    <div key={comm.id} className="flex gap-3 items-start">
                      ...
                      <div className="bg-card/60 border border-border p-3.5 rounded-2xl rounded-tl-none flex-1">
                        <div className="flex items-baseline gap-2 mb-1.5">
                          <span className="text-xs font-bold text-foreground">{comm.full_name || "Team Member"}</span>
                          <span className="text-[10px] text-muted-foreground/70">
                            {comm.created_at ? formatDistanceToNow(new Date(comm.created_at), { addSuffix: true }) : "Just now"}
                          </span>
                        </div>
                        <p className="text-xs text-foreground leading-relaxed">{comm.comment}</p>
                      </div>
                    </div>
                  ))
                )}
              ...
              <div className="p-4 border-t border-border/80 bg-background/40 flex gap-2 items-end shrink-0 relative">
                {showTagSuggestions && (
                  <div className="absolute bottom-[calc(100%+8px)] left-4 w-60 max-h-48 overflow-y-auto bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-50 p-1 flex flex-col gap-0.5">
                    ...
                    {mentionSuggestions
                      .filter(m => m.full_name.toLowerCase().includes(tagSearchQuery.toLowerCase()))
                      .map((member: any) => (
                        <button
                          key={member.id}
                    ...
                    {mentionSuggestions.filter(m => m.full_name.toLowerCase().includes(tagSearchQuery.toLowerCase())).length === 0 && (
                      <div className="text-[10px] text-muted-foreground text-center py-3">No members found</div>
                    )}
                  </div>
                )}
                <Textarea
                ...
                />
                <Button onClick={handlePostComment} disabled={!commentText.trim() || createComment.isPending}
                  size="sm" className="h-10 rounded-xl font-bold bg-blue-600 hover:bg-blue-500 text-white">
                  <Send className="h-3.5 w-3.5" />
                </Button>
              </div>
            </Card>
        ...
        {section === "activity" && (
          <div className="space-y-4 max-w-3xl animate-in fade-in duration-200">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Workspace Audit Trail</span>
            ...
            <Card className="border-border bg-card/30 backdrop-blur-md rounded-2xl p-6 shadow-md">
              {loadingActivity ? (
                <div className="text-center py-8 text-xs text-muted-foreground/70">Loading activity...</div>
              ) : activityArray.length === 0 ? (
                <div className="text-center py-12 text-muted-foreground">
                  <ActivityIcon className="h-10 w-10 mx-auto mb-3 opacity-20" />
                  <p className="font-bold text-foreground/80">No activity yet</p>
                  <p className="text-xs mt-1">All project actions will be logged here automatically.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {activityArray.map((act: any, idx: number) => (
                    <div key={act.id || idx} className="flex gap-3 text-xs leading-relaxed">
                      <div className="flex flex-col items-center">
                        ...
                        {idx < activityArray.length - 1 && <div className="w-px h-full bg-muted/40 mt-1" />}
                      </div>
                      <div className="pb-4 flex-1">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="font-bold text-foreground">{act.full_name || "Team Member"}</span>
                          <span className="text-muted-foreground">{act.action}</span>
                          {act.entity_name && (
                            <span className="text-primary font-bold">"{act.entity_name}"</span>
                          )}
                        </div>
        ...
        {section === "reports" && (
          <div className="space-y-6 max-w-7xl animate-in fade-in duration-200">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Burndown */}
              <Card className="border-border bg-card/30 backdrop-blur-md rounded-2xl p-6 shadow-md">
                <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-5">Task Burndown Curve</h3>
                <div className="h-64 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={[
                      { name: "Week 1", remaining: total, completed: 0 },
                      { name: "Week 2", remaining: Math.max(0, total - Math.floor(done * 0.2)), completed: Math.floor(done * 0.2) },
                      { name: "Week 3", remaining: Math.max(0, total - Math.floor(done * 0.5)), completed: Math.floor(done * 0.5) },
                      { name: "Week 4", remaining: Math.max(0, total - Math.floor(done * 0.8)), completed: Math.floor(done * 0.8) },
                      { name: "Current", remaining: Math.max(0, total - done), completed: done },
                    ]}>
                      <defs>
                      ...
                      <ChartTooltip contentStyle={{ backgroundColor: "#1e293b", borderColor: "#334155", borderRadius: "8px", fontSize: "11px" }} />
                      <Area type="monotone" dataKey="remaining" stroke="#3b82f6" strokeWidth={2} fillOpacity={1} fill="url(#colorRemaining)" name="Remaining" />
              ...
              <Card className="border-border bg-card/30 backdrop-blur-md rounded-2xl p-6 shadow-md">
                <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-5">Task Allocation Per Member</h3>
                <div className="h-64 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={involvedMembersList.map((m: any) => ({
                      name: (m.full_name || "Member").split(" ")[0],
                      completed: tasksArray.filter(t => t.assigned_to === m.id && (t.status === "done" || t.status === "completed")).length,
                      pending: tasksArray.filter(t => t.assigned_to === m.id && t.status !== "done" && t.status !== "completed").length,
                    }))}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#1f2937" opacity={0.3} />
                      <XAxis dataKey="name" stroke="#6b7280" style={{ fontSize: 9 }} />
                      <YAxis stroke="#6b7280" style={{ fontSize: 9 }} allowDecimals={false} />
                      <ChartTooltip contentStyle={{ backgroundColor: "#1e293b", borderColor: "#334155", borderRadius: "8px", fontSize: "11px" }} />
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                { label: "Overdue", value: tasksArray.filter((t: any) => t.due_date && new Date(t.due_date) < new Date() && t.status !== "done" && t.status !== "completed").length, color: "text-destructive" },
              ].map(stat => (
                <Card key={stat.label} className="border-border bg-card/40 rounded-2xl p-4 text-center shadow-sm">
                  <div className={cn("text-2xl font-bold", stat.color)}>{stat.value}</div>
                  <div className="text-[10px] text-muted-foreground/70 uppercase font-bold tracking-wider mt-1">{stat.label}</div>
                </Card>
              ))}
            </div>
          </div>
          ...
          <div className="max-w-2xl animate-in fade-in duration-200 space-y-5">
            <Card className="border-border bg-card/30 backdrop-blur-md rounded-2xl shadow-md">
              <div className="p-6 border-b border-border">
                <h4 className="text-sm font-bold text-foreground">Project Settings</h4>
                <p className="text-xs text-muted-foreground mt-1">Edit project parameters, status, dates, and color.</p>
              </div>
              <CardContent className="space-y-4 pt-5 text-xs">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={settingsStatus} onValueChange={setSettingsStatus} disabled={profile?.id !== project?.created_by}>
                ...
                <div className="space-y-1.5">
                  <Label className="text-[10px] font-bold text-muted-foreground uppercase">Color Tag</Label>
                  <div className="flex gap-2 flex-wrap">
                    {PROJECT_COLORS.map(c => (
                      <button key={c} onClick={() => setSettingsColor(c)}
                        className={cn("h-7 w-7 rounded-lg transition-all border-2", c, settingsColor === c ? "border-white scale-110" : "border-transparent opacity-70 hover:opacity-100")} />
                    ))}
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center pt-5 border-t border-border/60">
                  ...
                  <Button onClick={handleSaveSettings} disabled={updateProject.isPending}
                    className="text-xs font-bold bg-blue-600 hover:bg-blue-500 text-white rounded-xl h-9 px-5">
        ...
        <div className="fixed inset-y-0 right-0 w-[420px] bg-popover border-l border-border/75 shadow-2xl z-50 flex flex-col p-6 text-xs">
          <div className="flex items-center justify-between pb-4 border-b border-border mb-5 shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-muted-foreground font-bold">Task Detail</span>
            </div>
            <Button variant="ghost" size="icon" onClick={() => setSelectedTaskForDrawer(null)} className="h-7 w-7 text-muted-foreground hover:text-foreground rounded-xl bg-transparent">
              <X className="h-4 w-4" />
            </Button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-5">
            <div>
              <span className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest block mb-1">Task Title</span>
              <h3 className="text-sm font-bold text-foreground">{selectedTaskForDrawer.title}</h3>
            </div>

            {selectedTaskForDrawer.description && (
              <div>
                <span className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest block mb-1">Description</span>
            ...
            {selectedTaskForDrawer.delay_reason && (
              <div>
                <span className="text-[10px] font-bold text-red-400 uppercase tracking-widest block mb-1">Reason for Delay</span>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-muted/30 p-3 rounded-xl border border-border/60">
                <span className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-wider block mb-0.5">Status</span>
                <span className="text-xs font-bold text-foreground capitalize">{selectedTaskForDrawer.status?.replace(/_/g, " ")}</span>
              </div>
              <div className="bg-muted/30 p-3 rounded-xl border border-border/60">
                <span className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-wider block mb-0.5">Priority</span>
                <span className="text-xs font-bold capitalize">{selectedTaskForDrawer.priority || "normal"}</span>
              </div>

              <div className="bg-muted/30 p-3 rounded-xl border border-border/60">
                <span className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-wider block mb-0.5">Assignee</span>
                <div className="flex items-center gap-2">
                  <Avatar className="h-5 w-5 border border-border">
                    {selectedTaskForDrawer.assigned_to_avatar && <AvatarImage src={selectedTaskForDrawer.assigned_to_avatar} alt={selectedTaskForDrawer.assigned_to_name || ""} />}
                    <AvatarFallback className="text-[10px] font-bold bg-blue-500/10 text-primary">
                      {getInitials(selectedTaskForDrawer.assigned_to_name || getMemberName(selectedTaskForDrawer.assigned_to))}
                    </AvatarFallback>
                  </Avatar>
                  <span className="text-muted-foreground text-[10px]">{selectedTaskForDrawer.assigned_to_name || getMemberName(selectedTaskForDrawer.assigned_to)}</span>
                </div>
              </div>
              <div className="bg-muted/30 p-3 rounded-xl border border-border/60">
                <span className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-wider block mb-0.5">Due Date</span>
                <span className="text-xs font-bold text-foreground">
                  {selectedTaskForDrawer.due_date ? format(new Date(selectedTaskForDrawer.due_date), "MMM d, yyyy") : "No date"}
                </span>
              </div>
            </div>
          ...
          <div className="pt-4 border-t border-border mt-4 flex justify-between shrink-0">
            {(selectedTaskForDrawer.created_by === profile?.id || isAdmin) ? (
              <Button variant="ghost" onClick={() => handleDeleteTask(selectedTaskForDrawer.id, selectedTaskForDrawer.title)}
            ...
            <div className="flex items-center gap-2">
              <Button size="sm" onClick={() => { handleOpenEditTask(selectedTaskForDrawer); }}
                className="h-8 rounded-xl font-bold bg-transparent border border-border text-foreground hover:bg-accent text-xs gap-1.5">
                <Pencil className="h-3.5 w-3.5" /> Edit
              </Button>
              <Button size="sm" onClick={() => setSelectedTaskForDrawer(null)} className="h-8 rounded-xl font-bold bg-blue-600 hover:bg-blue-500 text-white">
                Close
              </Button>
            </div>
          </div>
      ...
      <Dialog open={showTaskDialog} onOpenChange={setShowTaskDialog}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto rounded-2xl border-border/80 bg-popover text-foreground">
          ...
          <div className="space-y-3.5 py-2 text-xs">
            ...
            <div className="p-3.5 rounded-xl border border-primary/10 bg-primary/5 flex items-center justify-between mt-2">
              <div className="space-y-0.5">
                <Label className="text-[10px] font-bold text-foreground uppercase">Allow Delegation</Label>
            ...
            <div className="space-y-1.5 pt-1">
              <div className="flex justify-between items-center text-[10px] font-bold text-muted-foreground uppercase">
                ...
                <span className="text-primary font-bold">{newTaskProgress}%</span>
              </div>
              <Slider
          ...
          <DialogFooter className="gap-2 border-t border-border/60 pt-3">
            <Button variant="outline" size="sm" onClick={() => setShowTaskDialog(false)} className="border-border text-foreground/80">Cancel</Button>
            ...
            <DialogContent className="sm:max-w-md rounded-2xl border-border/80 bg-popover text-foreground max-h-[85vh] overflow-y-auto">
              ...
              <div className="space-y-3.5 py-2 text-xs">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <Label className="text-[10px] font-bold text-muted-foreground uppercase">Priority</Label>
                    <Select value={editTaskPriority} onValueChange={setEditTaskPriority} disabled={!canEditDates}>
                      <SelectTrigger className="text-xs bg-muted border-border text-foreground"><SelectValue /></SelectTrigger>
                      <SelectContent className="bg-muted border-border">
                        {PRIORITY_OPTS.map(p => <SelectItem key={p.value} value={p.value}>{p.label}</SelectItem>)}
                      </SelectContent>
                    ...
                    {canModifyAssignment ? (
                      ...
                      <div className="flex h-10 w-full items-center justify-between rounded-md border border-red-500/20 bg-red-500/5 px-3 py-2 text-xs cursor-not-allowed opacity-60">
                        <span className="text-muted-foreground">
                          {editTaskAssignee
                            ? orgMembers.find((m: any) => m.id === editTaskAssignee)?.full_name || "Assigned User"
                            : "Unassigned"}
                        </span>
                      </div>
                    )}
                ...
                <div className="space-y-1.5 pt-1">
                  <div className="flex justify-between items-center text-[10px] font-bold text-muted-foreground uppercase">
                    <span>Task Progress</span>
                    <span className="text-primary font-bold">{editTaskProgress}%</span>
                  </div>
                  <Slider
                ...
                {(() => {
                  const isDelayed =
                    editTaskDueDate &&
                    new Date(editTaskDueDate).setHours(23, 59, 59, 999) < Date.now() &&
                    editTaskStatus !== "done" &&
              ...
              <DialogFooter className="gap-2 border-t border-border/60 pt-3">
                <Button variant="outline" size="sm" onClick={() => setEditingTask(null)} className="border-border text-foreground/80">Cancel</Button>
                <Button size="sm" onClick={handleSaveEditTask} disabled={updateTask.isPending}
                  className="bg-blue-600 hover:bg-blue-500 text-white font-bold">
      ...
      <Dialog open={showMilestoneDialog} onOpenChange={setShowMilestoneDialog}>
        <DialogContent className="sm:max-w-md rounded-2xl border-border/80 bg-popover text-foreground">
          <DialogHeader>
            <DialogTitle className="text-sm font-bold">Add Milestone</DialogTitle>
          </DialogHeader>
          <div className="space-y-3.5 py-2 text-xs">
            <div className="space-y-1.5">
              <Label className="text-[10px] font-bold text-muted-foreground uppercase">Milestone Name *</Label>
            ...
            <div className="space-y-1.5">
              <Label className="text-[10px] font-bold text-muted-foreground uppercase">Assignee</Label>
              <MemberSearchSelect
                members={orgMembers && Array.isArray(orgMembers) ? orgMembers : []}
                value={newMilestoneAssignee === "none" ? "" : newMilestoneAssignee}
          ...
          <DialogFooter className="gap-2 border-t border-border/60 pt-3">
            <Button variant="outline" size="sm" onClick={() => setShowMilestoneDialog(false)} className="border-border text-foreground/80">Cancel</Button>
      ...
      <Dialog open={showMemberDialog} onOpenChange={setShowMemberDialog}>
        <DialogContent className="sm:max-w-md rounded-2xl border-border/80 bg-popover text-foreground">
          <DialogHeader>
            <DialogTitle className="text-sm font-bold">Invite Team Member</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2 text-xs">
            <div className="space-y-1.5">
              <Label className="text-[10px] font-bold text-muted-foreground uppercase">Select Employee</Label>
              <Select value={newMemberId} onValueChange={setNewMemberId}>
                <SelectTrigger className="text-xs bg-muted border-border text-foreground"><SelectValue placeholder="Choose employee..." /></SelectTrigger>
                <SelectContent className="bg-muted border-border text-xs">
                  {(orgMembers as any[]).filter(m => !membersArray.some((pm: any) => pm.user_id === m.id)).map(m => (
            ...
            <div className="space-y-1.5">
              <Label className="text-[10px] font-bold text-muted-foreground uppercase">Project Role</Label>
          ...
          <DialogFooter className="gap-2 border-t border-border/60 pt-3">
            <Button variant="outline" size="sm" onClick={() => setShowMemberDialog(false)} className="border-border text-foreground/80">Cancel</Button>
            <Button size="sm" disabled={!newMemberId || addMember.isPending}
              onClick={() => addMember.mutate({ project_id: id!, user_id: newMemberId, role: newMemberRole }, {
                onSuccess: () => { setShowMemberDialog(false); setNewMemberId(""); refetchMembers(); }
              })}
              className="bg-blue-600 hover:bg-blue-500 text-white font-bold">
