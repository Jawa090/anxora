...
class EmailService {
  ...
  async sendPasswordResetEmail(email, resetToken, userName = '') {
    ...
    const mailOptions = {
      ...
      to: email,
      subject: 'Password Reset Request - ELINA',
      html: this.getPasswordResetTemplate(resetLink, userName, email),
      text: `
Hello ${userName || 'User'},

You have requested to reset your password for your ELINA account.
  ...
  getPasswordResetTemplate(resetLink, userName, email) {
    return `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e1e1e1; border-radius: 10px;">
            ...
            <p>We received a request to reset the password for your ELINA account associated with <strong>${email}</strong>.</p>
